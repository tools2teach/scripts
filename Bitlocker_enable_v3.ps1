...
[CmdletBinding()]
Param (
    [string]$SearchBase = "OU=Bitlocker_TEST,OU=Dallas,OU=Computer Accounts,DC=ais,DC=domain"
)
...
$ReportPath = Join-Path (Split-Path $MyInvocation.MyCommand.Path) -ChildPath "c:\temp\WorkstationsWithBitLocker.csv"
