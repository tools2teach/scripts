...
[CmdletBinding()]
    ...
    Param(
    [Parameter(Mandatory=$false, HelpMessage="OU=Clinton,OU=Computer Accounts,DC=ais,DC=domain", ValueFromPipelineByPropertyName=$true)]    
    [string]$OU,
    [Parameter(Mandatory=$false, HelpMessage="Enter path for log file, example: C:\temp\", ValueFromPipelineByPropertyName=$true)]    
    [string]$LogFilePath = ".\",
